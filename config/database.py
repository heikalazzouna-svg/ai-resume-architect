...
def _upgrade_admin_password(admin_id: int, password: str) -> None:
...
def verify_admin(email, password):
    """Verify admin credentials (supports bcrypt and legacy plaintext migration)."""
    conn = get_database_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        if not stored_password.startswith("$2b$") and not stored_password.startswith("$2a$"):
            _upgrade_admin_password(admin_id, password)
...
def add_admin(email, password):
    """Add a new admin with a bcrypt-hashed password."""
    conn = get_database_connection()
    cursor = conn.cursor()
...
def get_analysis_history(email):
    """Get chronological analysis history for a user by email address.

    Returns a list of dicts with keys: name, ats_score, keyword_match_score,
    format_score, section_score, missing_skills, recommendations, analyzed_at.
    """
    conn = get_database_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        JOIN resume_data r ON r.id = a.resume_id
        WHERE LOWER(r.email) = LOWER(?)
        ORDER BY a.created_at ASC
        ''', (email,))

        columns = [
            'name', 'ats_score', 'keyword_match_score',
            'format_score', 'section_score', 'missing_skills',
