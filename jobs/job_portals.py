...
class JobPortal:
    ...
    def search_jobs(self, job_title, location, experience=None):
        ...
        for portal in self.portals:
            ...
            if portal_name == "Foundit (Monster)":
                formatted_location = location.replace(' ', '+') if location else "Tunisia"
            elif portal_name == "Naukri":
                formatted_location = self.format_location(location) if location else "tunisia"
            elif portal_name == "Glassdoor":
                # For Glassdoor, keep spaces in location name
                formatted_location = location if location else "Tunisia"
            elif portal_name in ["LinkedIn", "Indeed", "TimesJobs"]:
                formatted_location = location.replace(' ', '%20') if location else "Tunisia"
            elif portal_name in ["FreshersWorld", "Instahyre"]:
                formatted_location = location.lower().replace(' ', '-') if location else "tunisia"
            else:
                formatted_location = location if location else "Tunisia"
            ...
            try:
                ...
                results.append({
                    ...
                    "icon": portal["icon"],
                    "color": portal["color"],
                    "title": f"{job_title} jobs in {location if location else 'Tunisia'}",
                    "url": url
                })
            except Exception as e:
